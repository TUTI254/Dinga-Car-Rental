                        ...
                        <Dialog.Panel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white text-left shadow-xl transition-all flex flex-col gap-5 p-6 ">
                            ...
                            <div className="flex-1 flex flex-col gap-3">
                                <div className="relative w-full h-40 bg-pattern bg-cover bg-center rounded-lg">
                                    <Image src={generateCarImageUrl(car,'angle')} alt="hero" fill className='object-contain' />
                                </div>
                                ...
                                <div className="mt-3 flex flex-wrap gap-4 ">
                                    {Object.entries(car).map(([key, value]) => (
                                        <div key={key} className="flex justify-between gap-5 w-full text-right">
                                            <h4 className="text-sm text-gray-400 capitalize">{key.split("_").join(" ")}</h4>
                                            <p className="text-black-100 text-sm font-semibold">{value}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
